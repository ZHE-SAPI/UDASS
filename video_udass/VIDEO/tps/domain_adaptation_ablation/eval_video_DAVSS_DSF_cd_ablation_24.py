...
def evaluate_domain_adaptation( models, test_loader, cfg,
    ...
    if cfg.TEST.MODE == 'video_single':
        ...
        eval_video_best(cfg, models, # eval_video_best_3frames   eval_video_best
                  device, test_loader, interp, fixed_test_size,
                  verbose)
...
def eval_video_single(cfg, models,
    ...
    for checkpoint, model in zip(['/home/ZZF/video_tps/TPS/experiments_result/snapshots/SynthiaSeq2CityscapesSeq_ACCEL_DeepLabv2_TPS_DAVSS_DSF_cd_ablation_24/model_38900.pth'], models):
        load_checkpoint_for_evaluation(model, checkpoint, device)
    # eval
    hist = np.zeros((cfg.NUM_CLASSES, cfg.NUM_CLASSES))
    for index, batch in tqdm(enumerate(test_loader)):
        ...
        with torch.no_grad():
            ...
            image_name = name.split('.')[0]
            # vis seg maps
            os.makedirs('/home/ZZF/video_tps/TPS/experiments_result/snapshots/SynthiaSeq2CityscapesSeq_ACCEL_DeepLabv2_TPS_DAVSS_DSF_cd_ablation_24/syn24', exist_ok=True)
            amax_output_col.save('%s/%s_color.png' % ('/home/ZZF/video_tps/TPS/experiments_result/snapshots/SynthiaSeq2CityscapesSeq_ACCEL_DeepLabv2_TPS_DAVSS_DSF_cd_ablation_24/syn24', image_name))
        label = label.numpy()[0]
...
def eval_video_best(cfg, models,
    ...
    assert osp.exists(cfg.TEST.DAVSS_DSF_cd_ablation_24[0]), 'DAVSS_DSF_cd_ablation_24 is not found'
    ...
    cache_path = osp.join(cfg.TEST.DAVSS_DSF_cd_ablation_24[0], 'all_res.pkl')
    ...
    # for i_iter in range(start_iter, max_iter + 1, step):
    restore_from = osp.join(cfg.TEST.DAVSS_DSF_cd_ablation_24[0], f'udass_syn_model_video_cnn.pth')
    ...
    # print([np.round(iou,1) for iou in computed_miou])
...
def eval_video_best_3frames(cfg, models,
    ...
    for i_iter in range(38900, max_iter + 1, step):
        restore_from = osp.join(cfg.TEST.DAVSS_DSF_cd_ablation_24[0], f'model_{i_iter}.pth')
        if not osp.exists(restore_from):
            # continue
            if cfg.TEST.WAIT_MODEL:
                print('Waiting for model..!')
