...
def train_TPS(daformer_model, model_tps, source_loader, target_loader, cfg, device):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP + 1)): 
        # 断点续训
        # for i_iter in tqdm(range(5499, cfg.TRAIN.EARLY_STOP + 1)): 
        ####  optimizer  ####
        optimizer.zero_grad()
        ...
        with torch.no_grad():
            ...
            try :
                ...
                try :
                    ...
                    classes_t = torch.Tensor([729]).float()
        ...
        loss = cfg.TRAIN.LAMBDA_T * (cfg.TRAIN.LAMBDA_SEG_MAIN * loss_trg + loss_tar_p) + loss_src_p + loss_sou + 0.01 * loss_mmd
        loss.backward()
