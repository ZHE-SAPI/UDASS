...
class ResNetMulti(nn.Module):
    ...
    def forward(self, cf, kf, flow, device, source='src'):
        cf = self.conv1(cf)
        cf = self.bn1(cf)
        cf = self.relu(cf)
        cf = self.maxpool(cf)
        ...
        flow_cf = (interp_flow2cf(flow) * interp_flow2cf_ratio).float().cpu().numpy()  # .float().cuda(device)


        kf_cpu = kf.cpu().numpy()
        kf_rec = np.zeros(kf.shape)
        rec_positions = np.zeros(kf.shape)
        for x in range(kf.shape[-1]):
            for y in range(kf.shape[-2]):
        ...
        kf_rec = torch.from_numpy(kf_rec).float().cuda(device)
        rec_positions = torch.from_numpy(rec_positions).float().cuda(device)


        pred = self.sf_layer(torch.cat((cf, kf_rec*rec_positions), dim=1))
        del kf_rec, rec_positions, flow_cf, kf
...
def get_accel_deeplab_v2_tps(num_classes=19, multi_level=True):
