...
def generate_experiment_cfgs(id):
    ...
    n_gpus = 1
    batch_size = 2
    iters = 40000 # for gta_transformer may can be 50000 for better results
    ...
    # -------------------------------------------------------------------------
        ...
        for architecture,                      backbone,  uda_hp in [
            ...
            for seed in seeds:
                uda, rcs_T, plcrop, opt, lr, schedule, pmult = uda_hp
                cfg = config_from_vars()
                cfg['checkpoint_config'] = dict(by_epoch=False, interval=500, max_keep_ckpts=-1)
                cfg['evaluation'] = dict(interval=500, metric='mIoU')
                cfgs.append(cfg)



    elif id == 821:    #  synthia  cnn
        seeds = [0]
        #        opt,     lr,      schedule,     pmult
        sgd   = ('sgd',   0.0025,  'poly10warm', False)
        adamw = ('adamw', 0.00006, 'poly10warm', True)
