...
def strongTransform_class_mix(image1, image2, label1, label2, mask_img, mask_lbl, cls_mixer, cls_list, strong_parameters, local_iter, vis = False, note = None):
    ...
    if mask_img.ndim == 2:
        mask_img, mask_lbl = mask_img[None, None, :, :], mask_lbl[None, None, :, :]

    print('mask_img.shape', mask_img.shape) # [1, 1, 1024, 1024]
    print('image1.unsqueeze(0).shape', image1.unsqueeze(0).shape) # [1, 3, 1024, 1024]
    print('image2.unsqueeze(0).shape', image2.unsqueeze(0).shape) # [1, 3, 1024, 1024]

    inputs_, _ = transformsgpu.oneMix(mask_img, data=torch.cat((image1.unsqueeze(0), image2.unsqueeze(0))))

    print('mask_lbl.shape', mask_lbl.shape) # [1, 1, 1024, 1024]
    print('label1.unsqueeze(0).shape', label1.unsqueeze(0).shape) # [1, 1024, 1024]
    print('label2.unsqueeze(0).shape', label2.unsqueeze(0).shape) # [1, 1024, 1024]

    _, targets_ = transformsgpu.oneMix(mask_lbl, target=torch.cat((label1.unsqueeze(0), label2.unsqueeze(0))))

    print('inputs_.squeeze(0).shape', inputs_.squeeze(0).shape) # 1, 3, 1024, 1024]
    print('targets_.squeeze(0).shape', targets_.squeeze(0).shape) # [1, 1, 1024, 1024]
...
class rand_mixer_cityscapes():
    def __init__(self, root, dataset):
        if dataset == "gta5":
            ...
            print('rand_mixer_cityscapes {} unsupported'.format(dataset))
            return
        self.root = root
        ...
        print('self.ids2img_dict[str(0)', random.sample(self.ids2img_dict[str(0)], 1))
    ...
    def mix(self, in_img, in_lbl, classes, local_iter, vis, note):
        ...
        for i in classes:
            ...
            if self.dataset == "cityscapes":
                while(True):
                    try:
                        # 随机选一个元素，元素是 [img_path, label_path]
                        img_path_rel, label_path_rel = random.choice(self.ids2img_dict[str(i)])
                        # 构造绝对路径
                        img_path = os.path.join(self.root, img_path_rel.replace("../dataset/Cityscapes/", ""))
                        label_path = os.path.join(self.root, label_path_rel.replace("../dataset/Cityscapes/", ""))
    ...
    def forward_train(self,
        ...
        if not self.source_only:
            ...
            if self.local_iter < 2:
                ...
                # target_tmpl
                gta5_cls_mixer = rand_mixer_cityscapes(get_data_path('cityscapes'), "cityscapes")
                class_to_select = [4, 6, 15, 17, 18] # select some hard to segmentation class (long tail classes) from the source domain(cityscapes)
                # class_to_select id_class {
                        # 0,   # road
                        # 1,   # sidewalk
                        # 2,  # building
                        # 3,  # wall
                        ...
                        # 18  # bicycle
                    # }    
                    # refer to https://github.com/mcordts/cityscapesScripts/blob/master/cityscapesscripts/helpers/labels.py
                ...
                if batch_size == 2:
                    ...
                    print('self.weak_target_img_tmpl[0].shape', self.weak_target_img_tmpl[0].shape) # [3, 1024, 1024]
                    print('mixed_img_ss[0].shape', mixed_img_ss[0].shape) # [3, 1024, 1024]
                    print('self.pseudo_label_tmpl[0].shape', self.pseudo_label_tmpl[0].shape) # [1024, 1024]
                    print('mixed_lbl_ss[0].shape', mixed_lbl_ss[0].shape) # [1024, 1024]
                    print('mix_masks_tmpl_t[0].shape', mix_masks_tmpl_t[0].shape) # [1, 1, 1024, 1024]



                    print('self.weak_target_img_tmpl[1].shape', self.weak_target_img_tmpl[1].shape) # [3, 1024, 1024]
                    print('mixed_img_ss[1].shape', mixed_img_ss[1].shape) # [3, 1024, 1024]
                    print('self.pseudo_label_tmpl[1].shape', self.pseudo_label_tmpl[1].shape) # [1024, 1024]
                    print('mixed_lbl_ss[1].shape', mixed_lbl_ss[1].shape) # [1024, 1024]
                    print('mix_masks_tmpl_t[1].shape', mix_masks_tmpl_t[1].shape) # [1, 1, 1024, 1024] or [1024, 1024]
                    ...
                    print('self.pseudo_weight_tmpl[0].shape', self.pseudo_weight_tmpl[0].shape) # [1024, 1024]
                    print('pseudo_weight_ss[0].shape', pseudo_weight_ss[0].shape) # [1024, 1024]
                    ...
                    print('self.pseudo_weight_tmpl[1].shape', self.pseudo_weight_tmpl[1].shape) # [1024, 1024]
                    print('pseudo_weight_ss[1].shape', pseudo_weight_ss[1].shape) # [1024, 1024]
                    ...
                    print('pseudo_weight_tss.shape', pseudo_weight_tss.shape)

                    print('self.weak_target_img_tmpl[0].shape', self.weak_target_img_tmpl[0].shape)
                    print('mixed_img_st[0].shape', mixed_img_st[0].shape)
                    print('self.pseudo_label_tmpl[0].shape', self.pseudo_label_tmpl[0].shape)
                    print('mixed_lbl_st[0].shape', mixed_lbl_st[0].shape)
                    print('mix_masks_tmpl_t[0].shape', mix_masks_tmpl_t[0].shape)



                    print('self.weak_target_img_tmpl[1].shape', self.weak_target_img_tmpl[1].shape)
                    print('mixed_img_st[1].shape', mixed_img_st[1].shape)
                    print('self.pseudo_label_tmpl[1].shape', self.pseudo_label_tmpl[1].shape)
                    print('mixed_lbl_st[1].shape', mixed_lbl_st[1].shape)
                    print('mix_masks_tmpl_t[1].shape', mix_masks_tmpl_t[1].shape)
                    ...
                    print('self.pseudo_weight_tmpl[0].shape', self.pseudo_weight_tmpl[0].shape)
                    print('pseudo_weight_st[0].shape', pseudo_weight_st[0].shape)

                    pseudo_weight0 = strong_Trans_form_(strong_parameters, self.pseudo_weight_tmpl[0], pseudo_weight_st[0])
                    strong_parameters['mix'] = mix_masks_tmpl_t[1]


                    print('self.pseudo_weight_tmpl[1].shape', self.pseudo_weight_tmpl[1].shape)
                    print('pseudo_weight_st[1].shape', pseudo_weight_st[1].shape)
                    ...
                    print('mixed_img_tss.shape', mixed_img_tss.shape) # [2, 3, 1024, 1024]
                    print('mixed_lbl_tss.shape', mixed_lbl_tss.shape) # [2, 1, 1024, 1024]
                    print('pseudo_weight_tss.shape', pseudo_weight_tss.shape) # [2, 1024, 1024]
                    ...
                    print('mixed_img_ss[0].shape', mixed_img_ss[0].shape) # [3, 1024, 1024]
                    print('self.pseudo_label_tmpl[0].shape', self.pseudo_label_tmpl[0].shape) # [1024, 1024]
                    print('mixed_lbl_ss.shape', mixed_lbl_ss.shape) # [1024, 1024]
                    print('mix_masks_tmpl_t[0].shape', mix_masks_tmpl_t[0].shape) # [1, 1, 1024, 1024]

                    mixed_img_tss0, mixed_lbl_tss0 = strongTransform_class_mix(self.weak_target_img_tmpl[0], mixed_img_ss[0], self.pseudo_label_tmpl[0],
                                                                    mixed_lbl_ss, mix_masks_tmpl_t[0], mix_masks_tmpl_t[0], gta5_cls_mixer, cls_to_use,
                                                                    strong_parameters, self.local_iter, vis = False, note = 'tss')
                    # mixed_img_tss1, mixed_lbl_tss1 = strongTransform_class_mix(self.weak_target_img_tmpl[1], mixed_img_ss[1], self.pseudo_label_tmpl[1],
                    #                                                     mixed_lbl_ss[1], mix_masks_tmpl_t[1], mix_masks_tmpl_t[1], gta5_cls_mixer, cls_to_use,
                    #                                                     strong_parameters, self.local_iter)
                    ...
                    print('self.pseudo_weight_tmpl[0].shape', self.pseudo_weight_tmpl[0].shape) # [1024, 1024]
                    print('pseudo_weight_ss.shape', pseudo_weight_ss.shape) # [1024, 1024]

                    pseudo_weight0 = strong_Trans_form_(strong_parameters, self.pseudo_weight_tmpl[0], pseudo_weight_ss)
                    # strong_parameters['mix'] = mix_masks_tmpl_t[1]
                    ...
                    mixed_img_tss = mixed_img_tss0
                    mixed_lbl_tss = mixed_lbl_tss0
                    pseudo_weight_tss = pseudo_weight0.squeeze(0)

                    # T2ST
                    strong_parameters['mix'] = None

                    print('pseudo_weight_tss.shape', pseudo_weight_tss.shape) # [1, 1024, 1024]

                    print('self.weak_target_img_tmpl[0].shape', self.weak_target_img_tmpl[0].shape) # [3, 1024, 1024]
                    print('mixed_img_st[0].shape', mixed_img_st[0].shape) # [3, 1024, 1024]
                    print('self.pseudo_label_tmpl[0].shape', self.pseudo_label_tmpl[0].shape) # [1024, 1024]
                    print('mixed_lbl_st.shape', mixed_lbl_st.shape) # [1024, 1024]
                    print('mix_masks_tmpl_t[0].shape', mix_masks_tmpl_t[0].shape) # [1, 1, 1024, 1024]


                    mixed_img_tst0, mixed_lbl_tst0 = strongTransform_class_mix(self.weak_target_img_tmpl[0], mixed_img_st[0], self.pseudo_label_tmpl[0],
                                                                        mixed_lbl_st, mix_masks_tmpl_t[0], mix_masks_tmpl_t[0], gta5_cls_mixer, cls_to_use,
                                                                        strong_parameters, self.local_iter, vis = False, note = 'tst')
                    # mixed_img_tst1, mixed_lbl_tst1 = strongTransform_class_mix(self.weak_target_img_tmpl[1], mixed_img_st[1], self.pseudo_label_tmpl[1],
                    #                                                     mixed_lbl_st[1], mix_masks_tmpl_t[1], mix_masks_tmpl_t[1], gta5_cls_mixer, cls_to_use,
                    #                                                     strong_parameters, self.local_iter)
                    ...
                    print('self.pseudo_weight_tmpl[0].shape', self.pseudo_weight_tmpl[0].shape) # [1024, 1024]
                    print('pseudo_weight_st.shape', pseudo_weight_st.shape) # [1024, 1024]

                    pseudo_weight0 = strong_Trans_form_(strong_parameters, self.pseudo_weight_tmpl[0], pseudo_weight_st)
                    # strong_parameters['mix'] = mix_masks_tmpl_t[1]
                    ...
                    mixed_img_tst = mixed_img_tst0
                    mixed_lbl_tst = mixed_lbl_tst0
                    pseudo_weight_tst = pseudo_weight0.squeeze(0)
                    print('mixed_img_tss.shape', mixed_img_tss.shape) # [1, 3, 1024, 1024]
                    print('mixed_lbl_tss.shape', mixed_lbl_tss.shape) # [1, 1, 1024, 1024]
                    print('pseudo_weight_tss.shape', pseudo_weight_tss.shape) # [1, 1024, 1024]
                    ...
                    print('mixed_img_tst.shape', mixed_img_tst.shape) # [1, 3, 1024, 1024]
                    print('mixed_lbl_tst.shape', mixed_lbl_tst.shape) # [1, 1, 1024, 1024]
                    print('pseudo_weight_tst.shape', pseudo_weight_tst.shape) # [1, 1024, 1024]
        ...
        # print('weak_img.shape', weak_img.shape) # [2, 3, 1024, 1024], [1, 3, 1024, 1024]
